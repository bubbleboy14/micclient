...
class MICSClient(Named):
    def __init__(self, host, port, verbose=False, name="anonymous", ai="", depth=1, book="", random=1, tiny=False, opponent=False, invisible=False, preppy=True, game=None, rofflim=3, dbuntil=30, rushbelow=240):
        ...
        self.ai = getPlayer(self.timer, self.move, self.displog, ai, book, depth, random, rofflim, dbuntil, rushbelow, preppy)
        ...
        if not invisible:
            ...
            self.display = Display(self.timer, self.move, self.promotion,
                self.draw, self.new, self.save, self.quit, self.seek, self.chat)
        self.color = None
    ...
    def refresh_time(self):
        ...
        if self.timer.get_opponent(self.color) < 0:
            self.timeout()
        self.display and self.display.draw_timers()
        return True
    ...
    def move(self, start, end, promotion=None):
        if self.color:
            self.log("move", self.board.fullmove, ":", start, end)
    ...
    def recv(self, data):
        ...
        if data.name == 'game':
            ...
            self.update_time(data.attr('white'), data.attr('black'))
