...
class Player(Named):
	def __init__(self, mover, outer, ai="simple", book="random", depth=1, random=1, rofflim=3, dbuntil=30):
		...
		try:
			if book:
				...
				book = '_nobook'
				bookinst = None
			self.ai = __import__("ai.%s"%(ai,),fromlist=["ai"]).Brain(depth, mover, outer, bookinst, random, rofflim, dbuntil)
...
def getPlayer(mover, outer, ai="simple", book="random", depth=1, random=1, rofflim=3, dbuntil=30):
	if not ai: return
